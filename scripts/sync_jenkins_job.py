...
class ScenarioConfig:
    ...
    script_path: str
    description: str
    # Per-scenario repo root — Path A and Path B live in different
    # gitlab subdirs. Default to path-a for backwards-compat.
...
SCENARIOS: dict[str, ScenarioConfig] = {
    ...
    # A2 — DROPPED from active rotation (see run_jenkins_build.py for
    # rationale). The variant tree stays as a reference layout but
    # is not synced to Jenkins by default.
    # Path B variants — AI-forged silver products. Their Jenkinsfile
    # lives one level deeper because the AI-generated workspace is
    # nested under ``variants/<scenario>/<workspace>/`` per the
    # path-b layout. The script_path reflects that nesting.
    "B1": ScenarioConfig(
        scenario="B1",
    ...
    "B2": ScenarioConfig(
        ...
        description="AI-forged in-workspace variant (B2). Jenkinsfile generated by fluid generate ci.",
        repo_subdir="path-b-ai-telco-silver-import-demo",
    ),
...
def container_repo_url(scenario_cfg: ScenarioConfig) -> str:
    ...
    Path A and Path B are two separate git roots; Jenkins pulls from
    each via the bind-mounted ``/workspace`` volume.
    """
    return f"/workspace/gitlab/{scenario_cfg.repo_subdir}/.git"
            ...
            <scm class="hudson.plugins.git.GitSCM">
              <userRemoteConfigs>
                <hudson.plugins.git.UserRemoteConfig>
                  <url>{escape(container_repo_url(scenario))}</url>
                </hudson.plugins.git.UserRemoteConfig>
              </userRemoteConfigs>
              <branches>
...
def main() -> None:
    ...
    # Use the per-scenario repo subdir so Path A and Path B variants
    # both resolve to the right git root.
