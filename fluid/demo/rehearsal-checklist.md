...
| Clone `~/gitlab/telco-silver-product-demo` | Off-stage prep | Greenfield workspace |
| Clone `~/gitlab/telco-silver-import-demo` | Off-stage prep | Existing dbt workspace |
...
| `task metadata:apply` | Live on-stage step | Applies Horizon metadata |
| `task metadata:verify` | Live on-stage step | Verifies comments, tags, contacts, and DMF hooks |
| Install `data-product-forge==0.7.10` in the GitLab workspace | Live on-stage step | Shows the released CLI |
...
| `fluid dmm publish --with-contract` | Live on-stage step | Marketplace / DMM close |
