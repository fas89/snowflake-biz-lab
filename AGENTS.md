...
Do not rename these scenarios casually. The launchpads, validation matrix, and demo story rely on this shared language. B1 is active through `task b1:forge:ai`, which performs a live Gemini/OpenAI forge and then applies lab guardrails for the external-reference Snowflake/dbt flow. B2 is active through `task b2:forge:mcp`, which reads the seeded Snowflake schema through forge-cli MCP and generates the contract, dbt project, Airflow DAG, and Jenkinsfile inside the Path B workspace.
